import type { Metadata } from "next";
import Script from "next/script";
import "./globals.css";
import ClientLayout from "./ClientLayout";

export const metadata: Metadata = {
  title: "Jaime.Capital - The Coin That Makes People Richer",
  description: "Jaime.Capital represents the ultimate wealth-building cryptocurrency. Built by the rich for the rich, this coin is designed to make people richer through innovative blockchain technology and strategic investment opportunities.",
  keywords: "Jaime.Capital, cryptocurrency, wealth building, investment, blockchain, DeFi, Solana, token, crypto, finance, rich, money",
  authors: [{ name: "Jaime.Capital Team" }],
  creator: "Jaime.Capital Team",
  publisher: "Jaime.Capital",
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
    },
  },
  openGraph: {
    title: "Jaime.Capital - The Coin That Makes People Richer",
    description: "Jaime.Capital represents the ultimate wealth-building cryptocurrency. Built by the rich for the rich, this coin is designed to make people richer through innovative blockchain technology and strategic investment opportunities.",
    url: "https://jaime.capital",
    siteName: "Jaime.Capital",
    images: [
      {
        url: "/jaime.jpg",
        width: 800,
        height: 600,
        alt: "Jaime.Capital Logo",
      },
    ],
    locale: "en_US",
    type: "website",
  },
  twitter: {
    card: "summary_large_image",
    title: "Jaime.Capital - The Coin That Makes People Richer",
    description: "Jaime.Capital represents the ultimate wealth-building cryptocurrency. Built by the rich for the rich, this coin is designed to make people richer.",
    images: ["/jaime.jpg"],
  },
  icons: {
    icon: [
      { url: "/favicon.ico" },
      { url: "/favicon.svg", type: "image/svg+xml" },
      { url: "/favicon-96x96.png", sizes: "96x96", type: "image/png" },
    ],
    apple: { url: "/apple-touch-icon.png", sizes: "180x180" },
  },
  manifest: "/site.webmanifest",
  metadataBase: new URL('https://jaime.capital'),
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <head>
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover" />
        <meta name="apple-mobile-web-app-title" content="$JAIME" />
        <link rel="icon" type="image/png" href="/favicon-96x96.png" sizes="96x96" />
        <link rel="icon" type="image/svg+xml" href="/favicon.svg" />
        <link rel="shortcut icon" href="/favicon.ico" />
        <link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png" />
        <link rel="manifest" href="/site.webmanifest" />
        {/* <Script id="font-loader" strategy="afterInteractive">
          {`
            (function() {
              try {
                var font = new FontFace('LisaStyle', 'url(data:font/woff2;base64,)');
                font.load().then(function(loadedFont){
                  document.fonts.add(loadedFont);
                  document.body.classList.add('font-loaded');
                }).catch(function(err){
                  console.warn('Font loading failed:', err);
                });
              } catch (e) {}
            })();
          `}
        </Script> */}
      </head>
      <body className="antialiased flex flex-col" style={{ minHeight: '100vh' }}>
        <ClientLayout>
          {children}
        </ClientLayout>
      </body>
    </html>
  );
}
